...
static void hal_io_init () {
    ...
    if (lmic_pins.rxtx != LMIC_UNUSED_PIN)
        IX02.pinMode(lmic_pins.rxtx, OUTPUT);
    ...
    for (uint8_t i = 0; i < NUM_DIO; ++i) {
        if (lmic_pins.dio[i] != LMIC_UNUSED_PIN) {
            ...
            IX02.pinMode(lmic_pins.dio[i], INPUT);
        }
    }
}
...
void hal_pin_rxtx (u1_t val) {
    if (lmic_pins.rxtx != LMIC_UNUSED_PIN)
        IX02.digitalWrite(lmic_pins.rxtx, val);
}
...
static void hal_io_check() {
    ...
    if (check_dio == 1) {
        uint8_t i;
        for (i = 0; i < NUM_DIO; ++i) {
            if (dio_states[i] != IX02.digitalRead(lmic_pins.dio[i])) {
...
static void hal_spi_init () {
    //SPI.begin();
    IX02.begin(3);
    IX02.pinMode(lmic_pins.nss, OUTPUT);
    IX02.pinMode(lmic_pins.rst, OUTPUT);
    IX02.digitalWrite(lmic_pins.nss, HIGH);
    IX02.digitalWrite(lmic_pins.rst, HIGH);
}

void hal_pin_nss (u1_t val) {
    
    IX02.digitalWrite(lmic_pins.nss, val);
    // if (!val)
